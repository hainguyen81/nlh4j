...
RUN echo [maven-build[$PROJECT_NAME] - dev,jdk$JDK_MAJOR_VERSION - MAVEN_REPOSITORY: $MAVEN_REPOSITORY] Build modules offline - Profiles dev,jdk$JDK_MAJOR_VERSION,issuedDep,licenseTool \
	&& mvn \
	--fail-never \
	-s $PROJECT_NAME/settings.xml -t $PROJECT_NAME/toolchains.xml \
	-Dmaven.repo.local=$MAVEN_REPOSITORY \
