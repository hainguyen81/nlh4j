...
CREATE OR REPLACE FUNCTION fn_iv_modules_by_user(IN codes text, IN ucd text)
RETURNS TABLE(
	id bigint
	, pid bigint
...
$BODY$
	...
	SELECT
			...
			, fn_iv_distinct_array(
					string_to_array( ( CASE	WHEN	( m.common OR fn_iv_is_system_admin( $2 ) )
								THEN	null
								ELSE	( '|' || COALESCE( (
												SELECT
														string_agg( fn.code, '|' )
