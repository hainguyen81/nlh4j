...
<c:if test="${resourceCache eq true}">
	<c:if test="${fn:length(theme) <= 0}">
		<link href="${resourcePath}/css/${src}"
		rel="stylesheet" media="${(empty media) ? 'screen' : media}"></link>
	</c:if>
	<c:if test="${fn:length(theme) > 0}">
		<link href="${resourcePath}/css/${theme}/${src}"
		rel="stylesheet" media="${(empty media) ? 'screen' : media}"></link>
	</c:if>
</c:if>
...
<c:if test="${resourceCache ne true}">
	<c:if test="${fn:length(theme) <= 0}">
		<link href="${resourcePath}/css/${src}?<%= System.currentTimeMillis() %>"
		rel="stylesheet" media="${(empty media) ? 'screen' : media}"></link>
	</c:if>
	<c:if test="${fn:length(theme) > 0}">
		<link href="${resourcePath}/css/${theme}/${src}?<%= System.currentTimeMillis() %>"
		rel="stylesheet" media="${(empty media) ? 'screen' : media}"></link>
	</c:if>
</c:if>
