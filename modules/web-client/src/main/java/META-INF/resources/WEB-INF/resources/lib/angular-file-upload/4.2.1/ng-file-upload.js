...
ngFileUpload.directive('backgroundImageBase64', ['$parse', '$timeout', function ($parse, $timeout) {
	return {
		restrict: 'AE',
		link: function (scope, elem, attr, file) {
			if (window.FileReader) {
				scope.$watch(attr.backgroundImageBase64, function(file) {
					if (file &&
							validate(scope, $parse, attr, file, null) &&
							(!window.FileAPI || navigator.userAgent.indexOf('MSIE 8') === -1 || file.size < 20000) &&
							(!window.FileAPI || navigator.userAgent.indexOf('MSIE 9') === -1 || file.size < 4000000)) {
						$timeout(function() {
							...
							fileReader.onload = function(e) {
								$timeout(function() {
									elem.attr('style', 'background-image: url(' + e.target.result + ')');
								});
							}
						});
					}
