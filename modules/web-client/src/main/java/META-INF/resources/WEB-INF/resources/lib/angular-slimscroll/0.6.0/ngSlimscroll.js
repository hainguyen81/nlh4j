...
(function () {
	...
	.directive(
			'ngSlimscroll',
			['$document', '$window', '$compile', '$timeout', '$log',
			 function($document, $window, $compile, $timeout, $log) {
				'use strict';
				...
				var setScopeValues = function (scope, element, attrs) {
					...
					scope.barZIndex = attrs.barZIndex || 9999;
				...
				return {
					...
					link: function(scope, element, attrs) {
						...
						var el = element[0];
						var barid = ('sb_' + (new Date()).getTime());
						...
						var minBarHeight = 10,
							...
							touchDiff,
							minBarWidth = 10,
							offHeight = el.offsetHeight,
							offWidth = el.offsetWidth;
...
//						});
						...
						if (!scope.horizontalScroll) {
							var bar = angular.element('<div id="' + barid + '" ng-mousedown="makeBarDraggable($event)""></div>');
							bar.css({
								'background': scope.color,
								'width': scope.size,
								'position': 'absolute',
								'top': 0,
								'margin-top': element.offset().top + 3,
								'margin-bottom': 3,
								'opacity': scope.opacity,
								'display': scope.alwaysVisible ? 'block' : 'none',
								'border-radius': scope.borderRadius,
								'z-index': scope.barZIndex,
								'cursor': 'pointer'
							});
							...
							var bar = angular.element('<div id="' + barid + '" ng-mousedown="makeBarDraggableHorizontal($event)"></div>');
							bar.css({
								'background': scope.color,
								'height': scope.size,
								'position': 'absolute',
								'left': element.offset().left,
								'margin-left': element.offset().left + 3,
								'margin-right': 3,
								'opacity': scope.opacity,
								'display': scope.alwaysVisible ? 'block' : 'none',
								'border-radius': scope.borderRadius,
...
//						element.wrap(wrapper);
						element.append(bar);
//						angular.element('body').append(bar);
						$compile(bar)(scope);
						...
						scope.getBarHeight = function () {
							...
							bar.css({ display: display });
			      			bar.data('hidden', (barHeight >= el.offsetHeight));
						};
						...
						scope.getBarWidth = function () {
							...
							bar.css({ display: display });
							bar.data('hidden', (barWidth >= el.offsetWidth));
						};
						...
						scope.onWheel = function (e) {
							if (typeof bar.data('hidden') === 'boolean' && bar.data('hidden') === true) return false;
						...
						if (scope.watchContent) {
							var contentWatcher = scope.$watch(
									function() {
										...
										//	clone.find('#' + barid).remove()
										//	return clone.html();

										// watch height/width changed
										return (!scope.horizontalScroll ? el.offsetHeight : el.offsetWidth);
									},
									function() {
										!scope.horizontalScroll && scope.getBarHeight();
										scope.horizontalScroll && scope.getBarWidth();
									}
							);
							scope.$on("$destroy", function () { contentWatcher(); });
						}

						scope.__initSlimScroll = function () {
