importModuleController(
		...
		function($scope, ajaxService, $rootScope) {
			...
			// Selection events
			// -------------------------------------------------
			$scope.onDestCheckChange = function(prop, item) {
				if (prop == 'insertable' && item[prop] == true) {
					item['viewable'] = true;
					item['updatable'] = true;
					item['deletable'] = true;
				}
				else if ((prop == 'updatable' && item[prop] == true)
						|| (prop == 'deletable' && item[prop] == true)) {
					item['viewable'] = true;
				}
				else if (prop == 'viewable' && item[prop] == false) {
