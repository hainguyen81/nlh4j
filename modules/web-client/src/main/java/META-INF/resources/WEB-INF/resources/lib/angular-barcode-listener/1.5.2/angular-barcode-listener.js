...
/*
 * Listen for scan with specified product prefix. @param [Function] onScan -
 * callback to call when scan is successful. Passes the scanned string. @param
 * [String] prefix - character prefix that appears before the scanned string
 * (e.g. 'P%', 'C%')
 */
...
exports.default = {
  /**
	 ...
	 *
	 * @param {String}
	 *            scanCharacteristics.barcodePrefix
	 * @param {Number}
	 *            [scanCharacteristics.barcodeLength] - if provided, the
	 *            listener will wait for this many characters to be read before
	 *            calling the handler
	 * @param {Number}
	 *            [scanCharacteristics.scanDuration]
	 * @param {Function}
	 *            scanHandler - called with the results of the scan
  ...
  onScan: function onScan() {
    ...
    console.log('onScan::prefix: [' + barcodePrefix + ']');
    console.log('onScan::length: [' + barcodeLength + ']');
    console.log('onScan::duration: [' + scanDuration + ']');
	 ...
	 * within the SwipeTrack browser. See "SwipeTrack Browser JavaScript
	 * Functions" section of SwipeTrack API:
	 * http://swipetrack.net/support/faq/pdf/SwipeTrack%20API%20(v5.0.0).pdf
	 */
    ...
    var swipeTrackHandler = function swipeTrackHandler(barcode) {
    	console.log('swipeTrackHandler::barcode: [' + barcode + ']');
    	if ((barcodePrefix || '').length <= 0) scanHandler(barcode);
    	else if (barcode.match('^' + barcodePrefix) !== null) scanHandler(barcode.slice(barcodePrefix.length));
    };
    ...
    var finishScan = function finishScan() {
    	console.log('finishScan::codeBuffer: [' + codeBuffer + ']');
      if (codeBuffer) {
        if (!barcodeLength) scanHandler(codeBuffer);
            ...
            codeBuffer = '';
            isScanning = false;
        	throw new TypeError('not correct required barcode length: ' + barcodeLength);
        }
      }
      scannedPrefix = '';
      codeBuffer = '';
    ...
    var keypressHandler = function keypressHandler(e) {
      ...
      if ((barcodePrefix || '') || (scannedPrefix === barcodePrefix && /[^\s]/.test(char))) {
        codeBuffer += char;
      } else if (scannedPrefix === expectedPrefix && char === barcodePrefix.charAt(charIndex)) {
        scannedPrefix += char;
      }
      console.log('keypressHandler::char: [' + char + ']::codeBuffer: [' + codeBuffer + ']');
    };
